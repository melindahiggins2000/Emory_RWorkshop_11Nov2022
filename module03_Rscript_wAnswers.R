...
ggplot(penguins) + 
  ...
  ggtitle("Penguins Flipper Length by Body Mass")
...
# this looks weird since there are 2 species 
# of penguins on the Biscoe island 
# - take out the bext fit line
...
# Use code above, 
# add color for year for the boxplots
...
# answers will vary - here is a simple example
# save each plot as an object
p4 <- ggplot(penguins, 
             aes(x=bill_length_mm, 
                 y=body_mass_g, 
...
p6 <- 
  ...
  geom_density(alpha=.2, fill="#FF6666") +
  facet_wrap(vars(species)) +
  ggtitle("Histograms of Bill Length (mm) by Species")
